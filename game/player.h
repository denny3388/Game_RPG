...
public:
    ...
    void getstatus()
    {
        cout << "你的 HP 為 [" << (hp<0?0:hp) << "/" << maxhp << "]， MP 為 [" << (mp<0?0:mp) << "/" << maxmp << "]" << endl;
    }
    void getability()
    {
...
bool player::exestat(monster& m)
{
    ...
    {
        miss*=0.7;
        cout << ">>此回合受到[遲鈍]效果影響，迴避率小幅下降!(剩下 " << --slow << " 回合)" << endl;
    }
    if(poison)
    {
        hp-=10;
...
void player::clearstat(){ //清除所有buff/debuff效果
   halfmp=0,noskill=-1,weak=0,slow=0,poison=0;
   protect=0;//女神庇佑
   good=0;//命運占卜-吉
   bad=0;//命運占卜-凶
   shade=0;//影子使者
